...
export default api => {
  ...
  if (api.userConfig.paas) {
    if (api.userConfig.paas.isSubApp) {
      // 增加导入入口
      api.addEntryImports(() => {
        return {
    ...
    api.modifyRoutes(routes => {
      ...
      // 修改路由，为路由默认加上前缀
      if (api.userConfig.paas.needModifyRoutes) {
        modifiedRoutes = modifyRoute(routes);
        tempRoutes = flattern(modifiedRoutes);
      } else {
        tempRoutes = flattern(routes);
      }
